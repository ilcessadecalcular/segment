...
class OnlyHRNetSeg(nn.Module):
    def __init__(self, config, in_feat=64, mid_feat=32, out_feat=1):
        ...
        self.up=nn.Sequential(
            ...
            nn.ConvTranspose2d(in_feat, mid_feat, kernel_size=2, stride=2),
            nn.Conv2d(mid_feat,mid_feat, 3, 1, 1),
            BatchNorm2d(mid_feat, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=False)
        )
        ...
        self.conv1=nn.Conv2d(mid_feat, mid_feat, 3, 1, 1)
        self.bn1 = BatchNorm2d(mid_feat, momentum=BN_MOMENTUM)
        self.conv2=nn.Conv2d(mid_feat, mid_feat, 3, 1, 1)
        self.bn2 = BatchNorm2d(mid_feat, momentum=BN_MOMENTUM)
    ...
    def forward(self,x):
        ...
        for out in outputs:
            real_out=self.up(out)
            real_out=self.conv1(real_out)
            real_out=self.bn1(real_out)
            real_out=self.relu(real_out)
            real_out=self.conv2(real_out)
            real_out=self.bn2(real_out)
            real_out=self.relu(real_out)

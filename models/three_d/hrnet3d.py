        ...
        for i in range(len(self.fuse_layers)):
            ...
            for j in range(1, self.num_branches):
                if i == j:
                    ...
                    d_output = x[i].shape[-3]
                    width_output = x[i].shape[-2]
                    height_output = x[i].shape[-1]
                    y = y + F.interpolate(
                        self.fuse_layers[i][j](x[j]),
                        size=[d_output, height_output, width_output],
...
class HRNetSeg(nn.Module):
    def __init__(self,config, **kwargs):
        ...
        self.last_layer = nn.Sequential(
            nn.Conv3d(
                ...
                stride=1,
                padding=0),
            nn.BatchNorm3d(last_inp_channels, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True),
            nn.Conv3d(
                in_channels=last_inp_channels,
    ...
    def forward(self, x):
        ...
        # Upsampling
        x0_d, x0_h, x0_w = x[0].size(2), x[0].size(3), x[0].size(4)
        x1 = F.interpolate(x[1], size=(x0_d, x0_h, x0_w), mode='trilinear', align_corners=ALIGN_CORNERS)
        x2 = F.interpolate(x[2], size=(x0_d, x0_h, x0_w), mode='trilinear', align_corners=ALIGN_CORNERS)
        x3 = F.interpolate(x[3], size=(x0_d, x0_h, x0_w), mode='trilinear', align_corners=ALIGN_CORNERS)
...
class Hrnet_3d(nn.Module):
    def __init__(self,config, out_feat=1):
        super(Hrnet_3d, self).__init__()
        self.config = config()
        self.hrnet = HRNetSeg(config)
        self.in_feat = config.DATASET.NUM_CLASSES
        self.up = nn.Sequential(
            nn.ConvTranspose3d(self.in_feat, self.in_feat, kernel_size=2, stride=2),
            nn.Conv3d(self.in_feat,self.in_feat,3,1,1),
            nn.BatchNorm3d(self.in_feat,momentum=BN_MOMENTUM),
        ...
        self.conv1 = nn.Conv3d(self.in_feat,self.in_feat,3,1,1)
        self.bn1 = nn.BatchNorm3d(self.in_feat,momentum=BN_MOMENTUM)
        self.last = nn.Conv3d(self.in_feat,out_feat,3,1,1)
        self.relu=nn.ReLU(inplace=True)
    def forward(self,x):
        out = self.hrnet(x)
        out = self.up(out)
